...
public class Client {
  ...
  /** Contains the messages which are buffered until the previous send operation has finished. */
  ...
  /**
   * Asynchronously closes the Websocket session. This will wait until all remaining messages have
   * been sent to the Client and then close the Websocket session.
   */
  public void close() {
  ...
  /**
   * Sends the given message asynchronously to the client. If there is already a async sending in
   * progress, then the message will be buffered and sent when possible.<br>
   * <br>
        ...
        if (isSendingMessage) {
          ...
          if (messagesToSend.size() >= 1000 || messagesToSendLength >= 1000000) {
            ...
            if (msg instanceof StringWebsocketMessage
              ...
              String concatenated =
                  ms.getString() + ";" + ((StringWebsocketMessage) msg).getString();
