...
public class CompressionResponseStream extends ServletOutputStream {
  ...
  public CompressionResponseStream(
      CompressionServletResponseWrapper responseWrapper, ServletOutputStream originalOutput) {
  ...
  /**
   * The threshold number which decides to compress or not. Users can configure in web.xml to set it
   * to fit their needs.
   */
   ...
   * Set debug level.
   *
   * @param debug The higher the number, the more detail shown. Currently the range 0 (none) to 3
   *     (everything) is used.
   */
  ...
  public void setCompressionMimeTypes(String[] compressionMimeTypes) {
    ...
    if (debug > 1) {
      System.out.println(
          "compressionMimeTypes is set to " + Arrays.toString(this.compressionMimeTypes));
    }
  }
  ...
  /**
   * Write <code>len</code> bytes from the specified byte array, starting at the specified offset,
    ...
    if (gzipstream == null) {
      ...
      if (response.isCommitted()) {
        ...
        response.setContentLength(
            -1); // don't use any preset content-length as it will be wrong after gzipping
