...
public class CompressionFilter extends GenericFilter {
  ...
  public void init() {
    ...
    if (str != null) {
      ...
      if (compressionThreshold != 0 && compressionThreshold < MIN_THRESHOLD) {
        if (debug > 0) {
          System.out.println(
              "compressionThreshold should be either 0 - no compression or >= " + MIN_THRESHOLD);
   ...
   *
   * <p>This method first examines the request to check whether the client support compression. <br>
   ...
   * then invokes the next entity in the chain using the FilterChain object (<code>chain.doFilter()
   * </code>), <br>
   */
  @Override
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (compressionThreshold == 0) {
      if (debug > 0) {
        System.out.println(
            "doFilter got called, but compressionThreshold is set to 0 - no compression");
      }
