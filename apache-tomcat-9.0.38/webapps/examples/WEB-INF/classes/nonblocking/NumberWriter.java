...
/**
 * This doesn't do anything particularly useful - it just writes a series of numbers to the response
 * body while demonstrating how to perform non-blocking writes.
 */
public class NumberWriter extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    NumberWriterListener listener =
        new NumberWriterListener(ac, req.getInputStream(), resp.getOutputStream());
  }

  /**
   * Keep in mind that each call may well be on a different thread to the previous call. Ensure that
   * changes in values will be visible across threads. There should only ever be one container
   * thread at a time calling the listener.
   */
